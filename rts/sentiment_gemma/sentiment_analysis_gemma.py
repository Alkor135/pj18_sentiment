"""Собирает gemma-based sentiment-оценки новостей из markdown-файлов через Ollama.

Скрипт читает настройки из `rts/settings.yaml` (секции `common` и
`sentiment_gemma`), строит жесткий промпт для Gemma, вызывает локальный
...
app = typer.Typer(help="Собирает sentiment-оценки новостей через локальную модель Gemma в Ollama.")
...
def cleanup_old_logs(log_dir: Path, max_files: int = 3) -> None:
    ...
    log_files = sorted(log_dir.glob("sentiment_analysis_gemma_*.txt"), key=lambda p: p.stat().st_mtime, reverse=True)
...
def setup_logging(ticker_label: str, verbose: bool = False) -> None:
    ...
    log_file = log_dir / f"sentiment_analysis_gemma_{timestamp}.txt"
    ...
    logging.info("[%s] Запуск sentiment_analysis_gemma. Лог: %s", ticker_label, log_file)


def load_settings() -> dict:
    """Загружает и нормализует настройки из common и sentiment_gemma."""
    raw = yaml.safe_load((TICKER_DIR / "settings.yaml").read_text(encoding="utf-8"))
    settings = {**(raw.get("common") or {}), **(raw.get("sentiment_gemma") or {})}
...
def main(
    ...
    model: Optional[str] = typer.Option(
        None,
        help="Локальная модель Ollama. По умолчанию берется из settings.yaml:sentiment_gemma.sentiment_model.",
    ),
