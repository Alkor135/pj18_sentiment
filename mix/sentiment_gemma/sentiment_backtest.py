"""
Backtest sentiment-стратегии Gemma на основе `sentiment_scores.pkl`.
...
- HTML-отчёты в `mix/sentiment_gemma/plots`
- XLSX с результатами в `mix/sentiment_gemma/backtest`
"""
...
app = typer.Typer(help="Backtest sentiment_gemma-стратегии по данным из PKL.")
...
def load_sentiment(path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise typer.BadParameter(
            ...
            "Запусти sentiment_analysis_gemma.py, чтобы дополнить pkl колонкой next_body."
        )
...
def index_by_date(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df["source_date"].duplicated().any():
        ...
        raise typer.BadParameter(
            ...
            "Перегенерируй pkl: sentiment_analysis_gemma.py теперь хранит одну строку на дату."
        )
    return (
...
def main(
    ...
    rules_yaml: Path = typer.Option(
        None,
        "--rules-yaml",
        help="YAML-файл с правилами. По умолчанию mix/sentiment_gemma/rules.yaml.",
    ),
    ...
    """Запускает полный backtest sentiment_gemma и сохраняет отчёты."""
    # --- Загрузка настроек из mix/settings.yaml (common + sentiment_gemma) ---
    _raw = yaml.safe_load((TICKER_DIR / "settings.yaml").read_text(encoding="utf-8"))
    settings = {**(_raw.get("common") or {}), **(_raw.get("sentiment_gemma") or {})}
