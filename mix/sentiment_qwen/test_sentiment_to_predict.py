...
from mix.sentiment_qwen.sentiment_to_predict import (
    load_settings,
    resolve_direction,
...
def test_load_settings_uses_sentiment_qwen_section(tmp_path: Path):
...
common:
  ...
  time_start: "21:00:00"
sentiment_qwen:
  predict_path: "C:/predict/{ticker_lc}_qwen"
  sentiment_output_pkl: "{ticker_lc}/sentiment_qwen/sentiment_scores.pkl"
""".lstrip(),
        encoding="utf-8",
    )
    ...
    assert settings["predict_path"] == "C:/predict/mix_qwen"
    assert settings["sentiment_output_pkl"] == "mix/sentiment_qwen/sentiment_scores.pkl"
...
def test_resolve_sentiment_pkl_adds_model_suffix_for_relative_base_path(tmp_path: Path):
    ...
    assert resolve_sentiment_pkl(settings, tmp_path) == tmp_path / "sentiment_scores_qwen3_14b.pkl"
